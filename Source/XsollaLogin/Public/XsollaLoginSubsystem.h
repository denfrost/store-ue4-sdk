...
public:
	 ...
	 * @param SuccessCallback Callback function called after successful user registration. Account confirmation message will be sent to the specified email.
	...
	/** Authenticate User
	 * Authenticates the user by the username and password specified via the authentication UI.
	 *
	 * @param Username Username. Required.
	 * @param Password Password. Required.
	...
	/** Internal request for token validation (called with each authentication update automatically)
	 *
	 ...
	 * Gets the URL for authentication via the specified social network.
	 *
	 ...
	 * @param BrowserWidget Widget to represent the social network authentication form. Can be set in the project settings.
	 * @param bRememberMe Whether the user agrees to save the authentication data. Default is 'false'.
	 */
	UFUNCTION(BlueprintCallable, Category = "Xsolla|Login")
	...
	* @param Scope Scope is a mechanism in OAuth 2.0 that limits application's access to a user's account. Can be "email", "offline", "playfab", or you own. If you pass you own scope, the Xsolla Login server doesn't process these values, but returns them in the received JWT. 
	* @param SuccessCallback Callback function called after the URL for social authentication was successfully received.
	* @param ErrorCallback Callback function called after the request resulted with an error.
	*/
	UFUNCTION(BlueprintCallable, Category = "Xsolla|Login")
	...
	/** Sets a new value of a token (used when the token is obtained by authenticating via a social network, etc.).
	 *
	 ...
	 *
	 * @param RefreshToken Token is used to refresh the expired access token. Received when authorizing users with username/password for the first time.
	 */
	UFUNCTION(BlueprintCallable, Category = "Xsolla|Login", meta = (AutoCreateRefTerm = "SuccessCallback, ErrorCallback"))
	void RefreshToken(const FString& RefreshToken, const FOnAuthUpdate& SuccessCallback, const FOnAuthError& ErrorCallback);

	/** Exchanges the user authentication code for a valid JWT. Works only when OAuth 2.0 is enabled.
	 *
	 ...
	 * Authenticates a user by exchanging platform specific session ticket for a token.
	 *
	 * @param ProviderName Platform where the session ticket was obtained.
	 ...
	 * @param SuccessCallback Callback function called after successful update of user attributes local cache.
	 * @param ErrorCallback Callback function called after the request resulted with an error.
	 */
	 ...
	 * Modifies the list of user attributes by creating/editing its items (changes are made on the server side).
	 *
	 * @param AuthToken User authorization token.
	 ...
	 * Removes user attributes with specified keys (changes are made on the server side).
	 *
	 * @param AuthToken User authorization token.
	 ...
	 *
	 * @param AuthToken User authorization token.
	 * @param SuccessCallback Callback function called after successful creation of the account linking code. The new linking code will be received.
	 * @param ErrorCallback Callback function called after the request resulted with an error.
	 */
	UFUNCTION(BlueprintCallable, Category = "Xsolla|Login", meta = (AutoCreateRefTerm = "SuccessCallback, ErrorCallback"))
	...
	*/
	 ...
	 *
	 * @param AuthToken User authorization token.
	 * @param Birthday User birth date in the (YYYY-MM-DD) format. Can be changed only once.
	 * @param FirstName User first name. Pass an empty string to remove the current first name.
	 * @param LastName User last name. Pass an empty string to remove the current last name.
	 * @param Gender User gender (f - for female, m - for male).
	 * @param Nickname User nickname. Pass an empty string to remove the current nickname.
	 ...
	 * @param SuccessCallback Callback function called after user phone number is modified successfully.
	 * @param ErrorCallback Callback function called after the request resulted with an error.
	 */
	UFUNCTION(BlueprintCallable, Category = "Xsolla|Login", meta = (AutoCreateRefTerm = "SuccessCallback, ErrorCallback"))
	 ...
	 * @param SuccessCallback Callback function called after successful modification of user friends data.
	 * @param ErrorCallback Callback function called after the request resulted with an error.
	 */
	UFUNCTION(BlueprintCallable, Category = "Xsolla|Login", meta = (AutoCreateRefTerm = "SuccessCallback, ErrorCallback"))
	 ...
	 *
	 * @param AuthToken User authorization token.
	 * @param Platform Name of a social provider. If empty, friends from all available social providers will be fetched.
	 ...
	 *
	 * @param Email User email used by the Xsolla SDK back-end service.
	 * @param SuccessCallback Callback function called after the access token is received successfully.
	 * @param ErrorCallback Callback function called after the request resulted with an error.
	 */
	 ...
	 * @param SuccessCallback Callback function called after user profile data is received successfully.
	 * @param ErrorCallback Callback function called after the request resulted with an error.
	 */
	UFUNCTION(BlueprintCallable, Category = "Xsolla|Login", meta = (AutoCreateRefTerm = "SuccessCallback, ErrorCallback"))
	 ...
	 *
	 * @param Nickname User nickname used as a search criteria.
	 * @param SuccessCallback Callback function called after user search is completed successfully.
	 * @param ErrorCallback Callback function called after the request resulted with an error.
...
public:
	 ...
	 *
	 * @param Token User authorization token.
	 * @param Parameter Name of parameter which value should be extracted from a token.
	 */
	UFUNCTION(BlueprintCallable, Category = "Xsolla|Login")
	...
	/** Gets social profiles associated with the specified user. */
	UFUNCTION(BlueprintCallable, Category = "Xsolla|Login")
	...
	/** Checks if the specified social network is linked to the user profile. */
	UFUNCTION(BlueprintCallable, Category = "Xsolla|Login")
	bool IsSocialNetworkLinked(const FString& Provider) const;

protected:
	/** Keeps the state of user login. */
	FXsollaLoginData LoginData;

	/** Social authentication URL to be opened in the browser. */
	...
	/** Cached list of user's social network friends that was obtained during the last UpdateSocialFriends method call. */
